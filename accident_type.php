  ...
  <div class="col-md-9 animated bounce">
    ...
    if (isset($_GET['type'])) {
      ...
      switch ($type) {
        case 'semi-annual':
          ...
          break;
        case 'reported_accident_type':
          // Fetch data for reported accident types
          $stmt = $conn->prepare("SELECT incident_type, COUNT(*) as count FROM reports GROUP BY incident_type");
          $stmt->execute();
          $result = $stmt->get_result();
          ...
          while ($row = $result->fetch_assoc()) {
            $incident_types[] = $row['incident_type']; // Store incident type
            $counts[] = $row['count']; // Store count for that incident type
          }
          break;
        // Other cases...
      }
    ...
    <script>
      ...
      const chart = new Chart(ctx, {
        ...
        data: {
          labels: <?php echo json_encode($incident_types); ?>, // Use PHP to output the incident types array
          datasets: [{
            label: 'Number of Reports by Accident Type',
