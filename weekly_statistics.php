...
// Initialize counts for all weeks (assuming 52 weeks in a year)
$counts = array_fill(0, 52, 0); // Weeks 1 to 52

// Fetch finished reports for each week
$stmt = $conn->prepare("SELECT WEEK(created_at) as week, COUNT(*) as count FROM reports WHERE status='finished' GROUP BY week");
$stmt->execute();
$result = $stmt->get_result();

while ($row = $result->fetch_assoc()) {
    $counts[$row['week'] - 1] = $row['count']; // Store count for that week
}
?>
...
<head>
  <meta charset="UTF-8">
  <title>Weekly Statistics - MDRRMO</title>
...
<div class="container-fluid">
  ...
  <div class="col-md-9 animated bounce">
    ...
    <canvas id="weekly-statistics-chart" width="400" height="200"></canvas>

    <script>
      const ctx = document.getElementById('weekly-statistics-chart').getContext('2d');
